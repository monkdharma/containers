...
# Function to print colored lines with precise formatting
print_line() {
  local color="$1"
  local text="$2"
  printf "${color}%s${RESET}\n" "$text"
...
print_line "${WHITE}" "| /workspace/data | Data Disk  | Fast   | Used for high IO requirement data.   |"
print_line "${WHITE}" "+------------------------------------------------------------------------------+"
...
print_line "${GREEN}" "CPU ：$PODWISE_CPU_COUNT"
print_line "${GREEN}" "Memory：$PODWISE_MEM_GB"
...
# Storage information
df_stats=$(df -ah)
print_line "${BLUE}" "Storage Information:"
 
# System Disk
disk=$(echo "$df_stats" | grep "/workspace$" | awk '{print $5" "$3"/"$2}')
print_line "${GREEN}" "System Disk (/workspace): $disk"
 
# Data Disk
if test -d "/workspace/data"; then
  disk=$(echo "$df_stats" | grep "/workspace/data" | awk '{print $5" "$3"/"$2}')
  if test -z "$disk" ; then
    disk="Not Mounted"
  fi
  print_line "${GREEN}" "Data Disk (/workspace/data): $disk"
fi
